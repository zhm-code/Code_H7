...
static uint8_t fdcan1_TX_Set_Speed_CW240[8] 		= {0x23,0xFF,0x60,0x00,0x00,0x00,0x80,0x40};	//23 FF 60 00 00 00 80 40	带载240RPM
static uint8_t fdcan1_TX_Set_Speed_CCW240[8] 		= {0x23,0xFF,0x60,0x00,0x00,0x00,0x80,0xC0};	//23 FF 60 00 00 00 80 C0	带载-240RPM
...
enum 
{
	...
	Set_Speed_CW240 	= 5,	//设置目标速度 +200 RPM 		
	Set_Speed_CCW240 	= 6,	//设置目标速度 -200 RPM 		
...
}GB5745_CAN_Step;	//发送步骤枚举




//设置开环运行
void vAPP_RunOpenLoop_GB5745X(uint32_t RunTime_ms,
    ...
    {  	
		...
		if(SET_MOTOR_CW == ucSetMotorRotation)
		{
			...
			if (  (ullCurrent_ms - ullCurrent_ms_100 >= 1000))
			{
				if (1 == IS_load )
				{
					FDCAN_TX_Send(FDCAN1_TxFrame,0x601, fdcan1_TX_Set_Speed_CW240, 0x08);
		...
		else
		{
			if ( (ullCurrent_ms - ullCurrent_ms_100 >= 1000))
			{
				if (1 == IS_load)
				{
					FDCAN_TX_Send(FDCAN1_TxFrame,0x601, fdcan1_TX_Set_Speed_CCW240, 0x08);
...
//完成复位
void vAPP_SetTestFinish_GB5745X(void)
{
	DAC_Set_Percentage(0);
	pid_pwm_set_GB5745X.fError 			= 0;
	pid_pwm_set_GB5745X.fOutput 		= 0;
...
//设置负载
void vAPP_SetLoad_GB5745X(uint32_t RunTime_ms, uint16_t usSetLoad)
{
    uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
...
//驱动器上电
void vAPP_DrivePowerOn_GB5745X(uint32_t RunTime_ms, uint16_t Set_Volt)
{
	uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
        ...
        else
        {
			if ( (IS_One_time_Program == 0) && (ullCurrent_ms - ullCurrent_ms_50 >= 100) )
			{
				...
				switch (One_time_Step)
				{
					...
					case Set_Speed_CW240:	//## 6. 设置目标速度 +240 RPM 		
					case Set_Speed_CCW240:  //## 6. 设置目标速度 -240 RPM 		
					{                       
						if (One_time_Step == Set_Speed_CW240)
						{
							FDCAN_TX_Send(FDCAN1_TxFrame,0x601, fdcan1_TX_Set_Speed_CW240, 0x08);
							
							
						}
						else if (One_time_Step == Set_Speed_CCW240)
						{
							FDCAN_TX_Send(FDCAN1_TxFrame,0x601, fdcan1_TX_Set_Speed_CCW240, 0x08);
...
//设置电机停止
void vAPP_SetMotorStop_GB5745X(uint32_t RunTime_ms,uint16_t Set_Volt)
{
    uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
...
//读取温度
void vAPP_ReadTmp_GB5745X(uint32_t RunTime_ms,uint16_t Set_Volt)
{
	uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
