...
//设置开环运行
void vAPP_RunOpenLoop_GB10847(uint32_t RunTime_ms,
                      ...
                      uint16_t Set_Volt)
{
    ...
    {  	
		...
		if(SET_MOTOR_CW == ucSetMotorRotation)
		{
            ...
            {
                SetMotorSpeed_AccDec(2000);
            }
            else
            {
                SetMotorSpeed_AccDec(10000);
            }
		...
		if(1 == IS_load)
        {
            if (RunTime_ms > 3000)
			{
				PID_Controller(1.5, uSysROParam.fRunOpenLoop_Curr, &pid_pwm_set_GB10847);
				DAC_Set_Percentage(42 + pid_pwm_set_GB10847.fOutput);
				osDelay(10);
				
			}
			else
			{
				DAC_Set_Percentage(42);
		...
		if(ullCurrent_ms % RunTime_ms < RunTime_ms - 500)
		{
			...
			if ( ((DriverBoard_DATA.RealSpd / 100 < 90) && (DriverBoard_DATA.RealSpd / 100 > 60) )
			 ||	((DriverBoard_DATA.RealSpd / 100 > -90) && (DriverBoard_DATA.RealSpd / 100 < -60) )
			)
			{
    ...
    else
    {
		...
		pid_pwm_set_GB10847.fError 			= 0;
		pid_pwm_set_GB10847.fOutput 		= 0;
		pid_pwm_set_GB10847.fProportion 	= 0;
		pid_pwm_set_GB10847.fIntegral 		= 0;
		pid_pwm_set_GB10847.fDerivative 	= 0;
...
//完成复位
void vAPP_SetTestFinish_GB10847(void)
{
	SetMotorSpeed_AccDec(0);
	pid_pwm_set_GB10847.fError 			= 0;
	pid_pwm_set_GB10847.fOutput 		= 0;
...
//设置负载
void vAPP_SetLoad_GB10847(uint32_t RunTime_ms, uint16_t usSetLoad)
{
    uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
...
//设置气缸
void vAPP_SetCylinder_GB10847(uint32_t RunTime_ms, uint16_t usSetCyl)
{
    uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
...
//驱动器上电
void vAPP_DrivePowerOn_GB10847(uint32_t RunTime_ms, uint16_t Set_Volt)
{
	uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
...
//编码器识别
void vAPP_EncoderIdentification_GB10847(uint32_t RunTime_ms)
{
	uint64_t ullCurrent_ms; //当前时间，相对于任务开始时
