...
void LCD_DispFlush(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint16_t *pData)
{
	...
	{
//		LCD_CMD = 0X2A; // 发送设置X坐标的指令
//		LCD_DATA = x >> 8; // X坐标的高8位
//		LCD_DATA = x ; // X坐标的低8位，因为指令读值只是低8位有效，所以等效于 x & 0XFF
//		
//		LCD_CMD = 0X2B; // 发送设置Y坐标的指令
//		LCD_DATA = nowY >> 8; // Y坐标的高8位
//		LCD_DATA = nowY ; // Y坐标的低8位，因为指令读值只是低8位有效，所以等效于 Y & 0XFF
		
		lcd_set_cursor(x, y);
;		
		lcd_wr_regno(0X2C);// 指令：开始写入GRAM
		for (uint16_t nowX = x; nowX <= width; nowX++) // 一行中，从左到右逐个像素
		{
			lcd_wr_data(*pData++); // 写入每个点的16位颜色数据, RGB565值
		}
	}
}
