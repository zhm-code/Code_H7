...
void Mount_FatFs(void)
{
	...
	printf("Mounting on the file system ...... \r\n");
	//尝试挂在文件系统
	FRESULT ret = f_mount(&User_FatFs, User_SDPath, 1);
	//挂在出错开始排查错误
	if(ret != FR_OK)
	{
		//没有有效的 FAT 卷，可能是文件系统损坏或未格式化
		if(ret == FR_NO_FILESYSTEM)
		{
			printf("Formatting the file system \r\n");
			//创建新的文件系统
			ret = f_mkfs(User_SDPath, FM_FAT32, 0, workBuffer, sizeof(workBuffer));
			if(ret == FR_OK)
			{
				printf("The file system is formatted successfully \r\n");
				printf("Mounting on the file system ...... \r\n");
				ret = f_mount(&User_FatFs, User_SDPath, 1);
				if(ret != FR_OK)
				{
				...
				{
					printf("The file system mounted successfully \r\n");
				}
			}
			else
			{
				printf("The file system is formatted failed \r\n");
...
void FatFs_DeleteFile(TCHAR *filename)
{
	...
	{
		printf("File deletion failed, error code：%u\r\n", res);
	}
}
