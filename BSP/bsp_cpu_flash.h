...
//开辟一个扇区给用户使用
extern const uint8_t para_flash_area[128*1024];
...
//写数据到CPU 内部Flash。 必须按32字节整数倍写。不支持跨扇区。扇区大小128KB. \
...
//擦除CPU FLASH一个扇区 （128KB)
extern uint8_t bsp_EraseCpuFlash(uint32_t _ulFlashAddr);
